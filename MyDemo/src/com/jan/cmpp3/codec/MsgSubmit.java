...
public class MsgSubmit extends MsgHead {
	...
	public byte[] toByteArry(){
		...
		try {
			...
			MsgUtil.writeString(dous,this.serviceId,10);//Service_Id 业务标识，是数字、字母和符号的组合。
			...
			MsgUtil.writeString(dous,this.feeTerminalId,32);//Fee_terminal_Id 被计费用户的号码
			...
			MsgUtil.writeString(dous,this.msgSrc,6);//Msg_src 信息内容来源(SP_Id)
			MsgUtil.writeString(dous,this.feeType,2);//FeeType 资费类别
			MsgUtil.writeString(dous,this.feeCode,6);//FeeCode
			MsgUtil.writeString(dous,this.valIdTime,17);//存活有效期
			MsgUtil.writeString(dous,this.atTime,17);//定时发送时间
			...
			MsgUtil.writeString(dous,this.linkID,20);//点播业务使用的LinkID
			dous.close();
		} catch (IOException e) {
